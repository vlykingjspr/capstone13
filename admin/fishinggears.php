...
<body>
                                    ...
                                    <tbody class="table-con" id="dataTable">
                                        ...
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo '</td>';
                                                echo '<td><button class="generate-pdf-btn btn btn-primary" data-id="' . htmlspecialchars($fgid) . '">Generate</button></td>';
                                                echo '</tr>';
                                            }
                                        } else {
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const generateButtons = document.querySelectorAll('.generate-pdf-btn');

            generateButtons.forEach(button => {
                button.addEventListener('click', function() {
                    const fg_id = this.getAttribute('data-id');

                    // Redirect to the generate_pdf.php file with the required ID as a query parameter
                    window.location.href = `generate_permit/fishing_gear/generate_gear_permit.php?fg_id=${fg_id}`;
                });
            });
        });
    </script>
